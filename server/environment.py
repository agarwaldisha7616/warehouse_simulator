...
from server.core.agents import (
    build_dynamic_obstacles,
    next_direction_from_grid,
    resolve_target_position,
)
from server.core.constants import TASK_CONFIGS
from server.core.engine import WarehouseEngine
...
class SmartWarehouseEnv(Environment):
    ...
    def _progress_score(self) -> float:
        ...
        delivered_ratio = self.engine.delivered_count / total_packages
        carrying_ratio = (
            sum(
                1
                for robot in self.engine.robots.values()
                if robot["carrying_id"] is not None
            )
            / total_packages
        )
        efficiency = max(
        ...
        score = delivered_ratio * 0.75 + min(carrying_ratio, 1.0) * 0.10 + efficiency * 0.15
        return max(0.0, min(1.0, score))
    ...
    def step(self, action: RobotAction, **kwargs) -> RobotObservation:
        ...
        previous_progress = self._progress_score()
        self.engine.steps += 1
        expired = self.engine.tick_deadlines()
        self._expired_package_count += len(expired)
        ...
        reward = max(0.0, min(1.0, max(progress_score, previous_progress)))
        return self._make_observation(round(reward, 3), " ".join(message_parts))
    ...
    def _make_observation(
        ...
        return RobotObservation(
            ...
            expired_package_count=self._expired_package_count,
            progress_score=self._progress_score(),
        )
